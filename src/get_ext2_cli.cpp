...
struct ReprojErrorIO {
  ...
  bool operator()(const T* const q_xyzw, const T* const t_xyz,
    ...
    // intr6: fx, fy, cx, cy, (k1 ignored), (k2 ignored)  -> 简化：只优化 fx,fy,cx,cy
    ...
    T fx=intr6[0], fy=intr6[1], cx=intr6[2], cy=intr6[3];
    T u = fx * (Pc.x()/Pc.z()) + cx;
    T v = fy * (Pc.y()/Pc.z()) + cy;
  ...
  static ceres::CostFunction* Create(const cv::Point2d& uv, const Eigen::Vector3d& Pl) {
    return new ceres::AutoDiffCostFunction<ReprojErrorIO, 2, 4, 3, 6>( new ReprojErrorIO(uv,Pl) );
  }
  cv::Point2d uv_; Eigen::Vector3d Pl_;
};

int main(int argc, char** argv) {
  std::string intr = (argc>1)? argv[1]: (std::string)PROJECT_SOURCE_DIR + "/config/intrinsic.txt";
  std::string ph   = (argc>2)? argv[2]: (std::string)PROJECT_SOURCE_DIR + "/data/corner_photo.txt";
  std::string ld   = (argc>3)? argv[3]: (std::string)PROJECT_SOURCE_DIR + "/data/corner_lidar.txt";
  std::string outT = (argc>4)? argv[4]: (std::string)PROJECT_SOURCE_DIR + "/config/extrinsic.txt";
  std::string outK = (argc>5)? argv[5]: (std::string)PROJECT_SOURCE_DIR + "/config/intrinsic.txt";

  CameraIntrinsics K; if(!load_intrinsics_txt(intr,K)){ std::cerr<<"Bad intrinsics\n"; return 1;}
  std::vector<PhotoCorners> photos; if(!load_photo_corners(ph, photos)){ std::cerr<<"Bad photo corners\n"; return 1; }
  std::vector<LidarCorners> lidars; if(!load_lidar_corners(ld, lidars)){ std::cerr<<"Bad lidar corners\n"; return 1; }
  ...
  double q_xyzw[4] = {0,0,0,1}, t_xyz[3] = {0,0,0};
  double intr6[6]  = {K.fx, K.fy, K.cx, K.cy, 0, 0};
  ...
  options.max_num_iterations=200; options.num_threads=4;
  ...
  Eigen::Matrix4d T; T.setIdentity(); T.block<3,3>(0,0)=R; T.block<3,1>(0,3)=t;
  if (!save_extrinsic_txt(outT, T)) { std::cerr<<"Save extrinsic failed\n"; return 1; }

  // 覆写 intrinsics（仅 fx,fy,cx,cy）
